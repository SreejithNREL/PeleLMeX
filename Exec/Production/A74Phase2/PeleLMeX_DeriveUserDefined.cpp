...
//
Vector<std::string> pelelmex_setuserderives()
{
...
//
void pelelmex_deruserdef (PeleLM* /*a_pelelm*/, const Box& bx, FArrayBox& derfab, int dcomp, int /*ncomp*/,
                        ...
                        const Geometry& geom, Real /*time*/, const Vector<BCRec>& /*bcrec*/, int /*level*/)
{
    ...
    amrex::ParallelFor(bx, [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept
    {
        ...
        if (rad > 0.0625 && rad < 0.0655 && z < (prob_lo[2]+0.002) ) {
          uder_arr(i,j,k,0) = 1.0;
        } else if (rad > 0.057 && rad < 0.060 && z < (prob_lo[2]+0.002) ) {
          uder_arr(i,j,k,0) = 1.0;
        } else if ( (std::abs(y-yc) > 0.065 || std::abs(x-xc) > 0.065) && z < (prob_lo[2]+0.002) ) {
          uder_arr(i,j,k,0) = 1.0;
        } else {
